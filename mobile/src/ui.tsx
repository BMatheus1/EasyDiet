...
export const SelectableCard = memo(function SelectableCard({
  ...
  return (
    <Pressable onPress={onPress} style={[styles.selectable, selected && styles.selectableActive]}>
      <View style={styles.selectableContent}>
        {icon ? (
          <View style={[styles.smallIcon, selected && styles.smallIconActive]}>
            <Ionicons name={icon} size={18} color={selected ? colors.paper : colors.forest} />
          </View>
        ) : null}
        <View style={styles.flex}>
          <Text style={[styles.selectableTitle, selected && styles.textWhite]}>{title}</Text>
          {subtitle ? <Text style={[styles.selectableSubtitle, selected && styles.textWhiteMuted]}>{subtitle}</Text> : null}
        </View>
        <Ionicons name={selected ? "checkmark-circle" : "ellipse-outline"} size={24} color={selected ? colors.paper : "rgba(23,33,29,0.32)"} />
      </View>
    </Pressable>
  );
});
...
export const IngredientCard = memo(function IngredientCard({ item, selected, onPress }: { item: Ingredient; selected: boolean; onPress: () => void }) {
  const superPremium = item.tier === "super_premium";
  return (
    <Pressable onPress={onPress} style={[styles.ingredientCard, selected && styles.ingredientCardActive]}>
      ...
      <View style={styles.ingredientMeta}>
        <StatusBadge label={labelFrom(tierLabels, item.tier)} tone={superPremium ? "gold" : item.tier === "premium" ? "forest" : "soft"} />
        <Text style={styles.ingredientHint}>{superPremium ? "Altera valor e prazo" : "Preco atualizado no resumo"}</Text>
      </View>
    </Pressable>
  );
});
...
export function SectionAccordion({ title, children, defaultOpen = false, right }: PropsWithChildren<{ title: string; defaultOpen?: boolean; right?: ReactNode }>) {
...
const styles = StyleSheet.create({
  ...
  selectableActive: { borderColor: colors.forest, backgroundColor: colors.forest },
  ...
  smallIconActive: { backgroundColor: "rgba(255,253,248,0.16)" },
